...
photos_segmented_path = str(pathlib.Path(__file__).parent / "photos_segmented_textures")


def get_texture(tex_name: str, rng=None, segment: bool = False, style: str = None) -> "Texture":
    ...
    if path not in Texture.tex_cache:
        Texture.tex_cache[path] = Texture(load_texture(oldpath, segment), tex_name=tex_name, rng=rng, style=style)

    return Texture.tex_cache[path]

def get_photos_segmented_texture(tex_name: str, rng=None, segment: bool = False, style: str = None) -> "Texture":
    paths = [f"{photos_segmented_path}/{tex_name}.jpg"]
    ...
    if path not in Texture.tex_cache:
        Texture.tex_cache[path] = Texture(load_texture(oldpath, False), tex_name=tex_name, rng=rng, style=style)
...
def load_texture(tex_path: str, segment: bool = False, segment_into_color=None, resize_factor=1):
    ...
    if resize_factor == 1:
        ...
        im = cv2.resize(im, None, fx=resize_factor, fy=resize_factor, interpolation=cv2.INTER_AREA)
