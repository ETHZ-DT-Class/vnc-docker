...
**IMPORTANT: since the vnc-docker repository and the docker image are linked to each other (the vnc-docker repo is used to generate the docker image), they will always share the same version number, and in theory you should keep them at the same version. To make things easier for you: regardless of the major or minor update type as described in the following paragraph, just run `git pull` (repo update) followed by either `make build` or `make pull` (image update).**
